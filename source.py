...
'''display(cdc_er_suicide_visits.info())
display(cdc_youth_suicide_attempt_rates.info())
display(cdc_adult_depression_rates.info())
display(cdc_state_depression_rates.info())
display(mha_state_of_mental_health_report_2024.info())
display(mha_state_of_mental_health_report_2023.info())'''
...
# This cell is converting the needed columns to numerics
...
'''display(cdc_er_suicide_visits.head())
display(cdc_youth_suicide_attempt_rates.head())
display(cdc_adult_depression_rates.head())
display(cdc_state_depression_rates.head())
display(mha_state_of_mental_health_report_2024.head())
display(mha_state_of_mental_health_report_2023.head())'''
...
cdc_state_depression_rates = cdc_state_depression_rates[
    cdc_state_depression_rates['State'].isin(valid_states)
]

state_merged = cdc_state_depression_rates.merge(mha_2023_and_2024,on="State",how="left")
...
x = state_merged.drop(columns=['Percent'])
...
num_features = ['Rank', 'Year']
cat_features = ['State', 'Demographics_Type', 'Demographics_Value']
...
# You can see above that I only used the Rank and Year features for the numerical pipeline and only State, Demographic Type, and Demographic Value for the catagorical data to clean up the pipelines a bit.
...
# Based on the above results Gradient Boosting will be the most accurate so I will move forward with that one for the state percentage guesses.
...
# 
# # Prior Feedback
# Unfortunately looking at my feedback branch I do not belive I received any peer feedback so there were no alterations based on that.
# 
# Based on my feedback from Checkpoint 2 I added some more correlations and a post cleaning table.
